...
const generateMockData = (
  ...
  return dateRange.map((date) => {
    let baseArmadilhasLuminosas: number;

    switch (rangeType) {
      case "day":
        baseArmadilhasLuminosas = Math.floor(Math.random() * 6);
        break;
      case "month":
        baseArmadilhasLuminosas = Math.floor(Math.random() * 41) + 10;
        break;
      case "year":
        baseArmadilhasLuminosas = Math.floor(Math.random() * 401) + 100;
        break;
      default:
        baseArmadilhasLuminosas = 0;
    }

    const variation =
      Math.random() > 0.7
        ? Math.floor(Math.random() * baseArmadilhasLuminosas * 0.5)
        : 0;
    const armadilhasLuminosas = baseArmadilhasLuminosas + variation;
...
export const ArmadilhasLuminosasChart = () => {
    ...
    <CardInfo>
      <ResponsiveContainer width="100%" height={200}>
        ...
        >
          ...
          <Bar
            dataKey="armadilhasLuminosas"
            fill="#3799d1"
            name="Encontrados em Armadilhas Luminosas"
            radius={[4, 4, 0, 0]}
          />
        </BarChart>
      </ResponsiveContainer>
